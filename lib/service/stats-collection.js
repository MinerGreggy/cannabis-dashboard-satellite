...
class StatsCollection {
  ...
  async init() {
    ...
    this.origin = 'cannabis-dashboard-satellite';
    ...
    this.connection.addService(constants.SERVICE('cannabis').walletUi);
  ...
  async updateRunningServices() {
    ...
    await Promise.all(this.enabledServices.filter(service => service !== plotterService).map(async service => {
      let isRunning = await this.daemonApiClient.isServiceRunning(constants.SERVICE('cannabis')[service]);
